<template>
  <div class="content">
    <van-cell-group>
      <van-cell title="场地选择" :value="field.value" :value-class="{'no_data': !field.placeId}" size="large" />
    ...
    <div class="collapse_content" v-if="field.placeId  && dateList.length > 0">
      ...
      <div v-if="!(appointList&& appointList.length>0)" class="no_data">
        <div></div>
        <p>暂无数据~</p>
      </div>
    </div>
...
import {
  queryTheoryPlace, theoryPlaceAdvanceDaysInfo, queryStudentProgress, theoryAppointTimeLoad, theoryAppoint
} from './api';

export default {
  setup() {
    setBanBanTitle('理论课预约');
    ...
    const appointInfo = async (date: any) => {
      const res = await theoryAppointTimeLoad({
    ...
    const getDateList = async (placeId: any) => {
      ...
      const res: any = await theoryPlaceAdvanceDaysInfo({
        placeId
      });
      dateList.value = res || [];
      if (res && res.length > 0) {
        const maxDateObj = res[res.length - 1] || {};
        const minDateObj = res[0] || {};
    ...
    const getAnalogPlaceList = async () => {
      const res: any = await queryTheoryPlace();
      console.log('res', res);
      // placeList.value = res?.infos || [];
      field.value = res.venueName;
      field.placeId = res.venueId;
