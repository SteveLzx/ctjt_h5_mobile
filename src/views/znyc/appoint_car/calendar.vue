...
function queryAppointadvanceDays(data: any, isCoach = false) {
  const url = isCoach ? 'teaching/v1/coach/querySurplusNumInfo' : 'teaching/v1/queryAppointadvanceDays';
  return new Promise(resolve => {
    $http.post(url, data).then(res => resolve(res));
  });
}
...
export default defineComponent({
  ...
  setup(props: any, context: any) {
    ...
    watch(props, async () => {
      ...
      const res = await queryAppointadvanceDays(data, props.isCoach);
      // 后台返回每天的预约情况
