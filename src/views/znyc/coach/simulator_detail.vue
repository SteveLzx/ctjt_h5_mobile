...
<div style="overflow: hidden;height:100%">
  ...
  <div class="tab_box">
    <ul class="tab_list">
      <li class="tab" @click="tabTaggle(6)" :class="{'active': tabFlag === 6}">C1学员<p>(已预约{{reserveInfoC1.alreadyAppointNums}}/{{reserveInfoC1.analogNums}}辆)</p></li>
      <li class="tab" @click="tabTaggle(7)" :class="{'active': tabFlag === 7}">C2学员<p>(已预约{{reserveInfoC2.alreadyAppointNums}}/{{reserveInfoC2.analogNums}}辆)</p></li>
    </ul>
  </div>
      ...
      <div class="data_scroll_box" v-if="reserveDetail.list6.length > 0">
        <van-swipe-cell v-for="item in reserveDetail.list6" :key="item.userId" :class="{'mine_style': item.isBelongToCoach}">
          <ul class="table_body">
            ...
            <li>{{item.learnPhaseAliasName}}</li>
            <li>
              {{item.itemTotalPracticeTime}}小时
            </li>
            <li>{{item.analogTotalPracticeTime}}小时</li>
...
import { queryAnalogReserveInfo, queryAnalogReserveInfoBytimeRange, queryAnalogReserveDetail } from './api';
...
export default {
  setup() {
    ...
    const chooseTime = async (driveType: number) => {
      ...
      // 查询当前时间段下的预约情况
      const reserveInfo = await queryAnalogReserveInfoBytimeRange(data);
      ...
      reserveDetail.value[`list${driveType}`] = await queryAnalogReserveDetail(data1);
      // 点击确认后复制当前选择的日期和时间
