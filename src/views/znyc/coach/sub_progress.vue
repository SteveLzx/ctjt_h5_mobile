...
<template>
    <div style="background: #f5f3f5;height:100%">
      <ul class="item_list">
        <li>
          训练场
          <span>{{currentVenue.venueName || '暂无'}}</span>
        </li>
      </ul>
            ...
            v-for="(item, index) in appointmentInfo" :key="index" :id="'list_' + index">
              <div class="time_quantum" @click="toggleList(item, index)" :class="{'active': currentTime === item.timeRange, 'has_mine': item&&Number(item.coachStuNums) }">
                ...
                <span class="right_text">
                  <span class="status" v-if="item.totalAlreadyAppointNums === 0 || Number(item.totalRealityAppointNums) > Number(item.totalAlreadyAppointNums)">
                    {{item.totalAlreadyAppointNums}}/{{item.totalRealityAppointNums}}辆
                  </span>
              ...
              <ul>
                <li class="car_model" v-for="val in item.list" :key="val.driveType">
                  ...
                  <div class="progress_line">
                    <p>已预约{{val.alreadyAppointNums}}/{{val.totalRealityAppointNums}}</p>
                    <div class="split">
                      <div class="percentage" :class="{'full': val.alreadyAppointNums !== 0 && val.totalRealityAppointNums === val.alreadyAppointNums}"
                      :style="{width: val.alreadyAppointNums === 0 ? 0 : (val.alreadyAppointNums/val.totalRealityAppointNums * 100) + '%'}"></div>
                    </div>
                  </div>
...
import MyCalendar from '../appoint_car/calendar.vue';
import { queryBookableDate, queryReserveInfo, queryReserveInfoBytimeRange } from './api';
...
export default {
  setup() {
    ...
    const queryByTimeRange = async (item: DaysType = {}) => {
      ...
      const modelList = await queryReserveInfoBytimeRange(sendData);
      const copyItem = item;
    ...
    const toDetail = (type: number) => {
      ...
      proxy.$root.$router.push({ path: '/coach/sub_detail', query });
    };
