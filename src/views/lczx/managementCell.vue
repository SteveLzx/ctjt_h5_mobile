...
<div class="content" v-if="authFlag">
  <div class="top_content">
    <div :class="['radio_box']" @click="goToCapitalManagement">
      <div class="icon1"></div>
      <span class="radio_title">资金监管</span>
      <span>

        <van-icon name="arrow" color="#9c9c9c" />
      </span>
    </div>
    <div :class="['radio_box']"  @click="goToPeriodManagement">
      <div class="icon2"></div>
      <span class="radio_title">学时管理</span>
      <span>
...
export default {
  setup() {
    ...
    const goToPeriodManagement = () => {
      proxy.$root.$router.push({
        path: '/coach/periodManagement',
      });
    };

    const goToCapitalManagement = () => {
      proxy.$root.$router.push({
    ...
    return {
      goToPeriodManagement,
      goToCapitalManagement,
      authFlag
    };
  },
  components: {
